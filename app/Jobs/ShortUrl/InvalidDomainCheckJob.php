...
class InvalidDomainCheckJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $logPrefix = 'InvalidDomainCheckJob: ' . $campaignName . ' - ';

        info($logPrefix . 'started');

        try {
            ShortUrl::query()
                ->select(['id', 'campaign_id', 'status', 'original_domain', 'expired_at'])
                ->where([
                    'campaign_id' => $campaignId,
                    'status' => ShortUrlConstant::INVALID,
                ])
                ->lazyById(1000, 'id')
                ->each(function ($shortUrl) use ($campaignName, $logPrefix) {
                    $now = now();
                    ...
                    info($logPrefix . ($status === ShortUrlConstant::EXPIRED ? 'Expired' : '') . $shortUrl->id . ' - ' . $originalDomain);
                });
